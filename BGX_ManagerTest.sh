...
show_mainMenu() {
	...
	echo "~~~~~~~~~~~~~~~~~~~~~"
  echo -e "1 - My Masternodes Status"
  echo -e "2 - Install or Add Masternodes"
  echo -e " - Masternode Manager"
  echo -e " - Donate"
	echo -e "X - Exit"
}
...
read_MainMenuOptions(){
	...
	case $choice in
		1) status_BGXMasternodes ;;
		2) install_BGXMasternodes ;;
		x) exit 0;;
		*) echo -e "${RED}Error...${STD}" && sleep 2
	esac
}
install_BGXMasternodes(){
  ...
  echo -e ${YELLOW}"Vultr and Digital Oceans Users - Check Support IPv6 on initial Server Selection"${CLEAR}
  echo
  echo -e ${GREEN}"Please enter the total number (1-8) of $COIN Masternode(s) you want"${CLEAR}
  echo -e ${YELLOW}" -If your VPS doesn't support IPv6 press 1 for 1 IPv4 Masternode"${CLEAR}
