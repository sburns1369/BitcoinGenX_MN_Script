...
declare -i NC BGX BGXC BGXOld BGXMN0 BGXMN1 BGXMN2 BGXMN3 BGXMN4 BGXMN5 BGXMN6 BGXMN7 BGXMN8 BGXMN9 BGXMN10
#Counter
NC=0
...
# manager_BGXMasternodes menu
manager_BGXMasternodes(){
  clear
  echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
local choice
  ...
  ${COINDAEMONCLI} -datadir=${COINHOME}${nodeunit}/${COINCORE} masternode status &> ${DPATH}BGXMN${nodeunit}.tmp
    ...
    echo -e ${YELLOW} "Masternode is still loading block Index, please wait." ${CLEAR}
      ...
      ${COINDAEMONCLI} -datadir=${COINHOME}${nodeunit}/${COINCORE} masternode status &> ${DPATH}BGXMN${nodeunit}.tmp
      DISPIP=$(sed -n '4p' < /usr/local/nullentrydev/BGXMN${nodeunit}.tmp | cut -d'"' -f4 | cut -d':' -f1)
        ...
        DISPIP=$(sed -n '4p' < /usr/local/nullentrydev/BGXMN${nodeunit}.tmp | cut -d'"' -f4 | cut -d':' -f1-8)
          echo -e "Running on IPv6 : ${YELLOW} ${DISPIP}" ${CLEAR}
        fi
        rm -r /usr/local/nullentrydev/BGXMN${nodeunit}.tmp
