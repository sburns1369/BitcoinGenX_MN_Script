...
DOWNLOADCOINFILES=https://github.com/BitcoinGenX/BitcoinGenesisX/files/2896837/bitcoingenx-linux-static.zip
COINFILES=bitcoingenx-linux-static.zip
DECOMPRESS='unzip'
#rocketstrap
NEBootStrap=http://nullentry.com/chain/BGX/rocketstrap.rar
AddNodeList=http://nullentry.com/chain/BGX/addnodes.tbl
ADDNODE0=66.42.113.222:4488
ADDNODE1=108.61.179.198:4488
ADDNODE2=206.189.227.156:4488
ADDNODE3=155.138.207.17:4488
ADDNODE4=5.189.163.30:4488
ADDNODE5=213.227.154.56:4488
ADDNODE6=23.106.215.65:4488
ADDNODE7=155.138.207.17:4488
ADDNODE8=45.76.3.217:4488
ADDNODE9=8.12.18.113:4488
COINPORT=4488
COINRPCPORT=19021
...
bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/BitcoinGenX_MN_Script/master/null_logo.sh)
pause
}
#bitcoingenx_Logo
Function_Display_Bitcoingenx_Logo(){
  bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/BitcoinGenX_MN_Script/master/logo.sh)
  pause
  }
#Logo(){
Function_Rocket_Delay(){
bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/BitcoinGenX_MN_Script/master/rocket.sh)
}
  ...
  bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/BitcoinGenX_MN_Script/master/welcome.sh)
  ...
  # Function_Manager_BGXMasternodes menu
  Function_Manager_BGXMasternodes(){
  clear
  echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  ...
  echo -e "X -  Exit"
  Function_Read_Manager_BGXMasternodes
  }

  # manager_BGXMasternodes read options
  Function_Read_Manager_BGXMasternodes(){
  local choice
  read -p "Enter choice " choice
  case $choice in
...
case $choice in
  ...
  *) echo -e "${RED}Error...${STD}" ${CLEAR} && sleep 2
  ...
  if [ -d /home/${COINl}${nodeunit} ]; then
    if [ -z ${nodeunit} ]; then
    echo -e ${GREEN}"Found BGX-Oldnode Installation Found - /home/${COINl}" ${CLEAR}
  else
    echo -e ${GREEN}"Found BGX-${nodeunit} Installation Found - /home/${COINl}${nodeunit}" ${CLEAR}
  fi
  ${COINDAEMONCLI} -datadir=${COINHOME}${nodeunit}/${COINCORE} masternode status &> ${DPATH}BGXMN${nodeunit}.tmp
    if grep -q "Hot node, waiting for remote activation" ${DPATH}BGXMN${nodeunit}.tmp; then
      echo -e ${YELLOW} "Masternode Ready, waiting for activation from Wallet" ${CLEAR}
    elif grep -q "Loading block index..." ${DPATH}BGXMN${nodeunit}.tmp; then
    echo -e ${YELLOW} "Masternode is still loading block Index, please wait." ${CLEAR}
    elif grep -q "Masternode successfully started" ${DPATH}BGXMN${nodeunit}.tmp; then
      echo -e ${GREEN} "Masternode Successfully Started" ${CLEAR}
    elif grep -q "Masternode not found in the list of available masternodes. Current status: Node just started, not yet activated" ${DPATH}BGXMN${nodeunit}.tmp; then
      echo -e ${YELLOW} "Masternode is loading blocks, Please Wait " ${CLEAR}
    elif grep -q "error: couldn't connect to server" ${DPATH}BGXMN${nodeunit}.tmp; then
      ...
      DISPIP=$(sed -n '4p' < /usr/local/nullentrydev/${nodeunit}.tmp | cut -d'"' -f4 | cut -d':' -f1)
        if [[ "$DISPIP" =~ (([01]{,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5])\.([01]{,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5])\.([01]{,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5])\.([01]{,1}[0-9]{1,2}|2[0-4][0-9]|25[0-5]))$ ]]; then
          ...
          # /dev/fd/63: line 1032: [: missing `]' trace appears to put it here.
...
Function_Glances(){
  ...
  #attempt to move legacy masternode
    bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/BitcoinGenX_MN_Script/master/donations.sh)
    pause
  }
